...
export default {
	...
	actions: {
		init({ dispatch, rootGetters }) {
			console.log('Vuex module: twilightproject.validitygadgetchain.validitygadgetchain initialized!')
		...
		async QuerySnapshotProposals({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				...
				while (all && (<any> value).pagination && (<any> value).pagination.next_key!=null) {
					let next_values=(await queryClient.querySnapshotProposals({...query, 'pagination.key':(<any> value).pagination.next_key})).data
					value = mergeResults(value, next_values);
				}
				commit('QUERY', { query: 'SnapshotProposals', key: { params: {...key}, query}, value })
				if (subscribe) commit('SUBSCRIBE', { action: 'QuerySnapshotProposals', payload: { options: { all }, params: {...key},query }})
		...
		async sendMsgSnapshotProposal({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				...
				const msg = await txClient.msgSnapshotProposal(value)
				const result = await txClient.signAndBroadcast([msg], {fee: { amount: fee, 
	gas: "200000" }, memo})
				return result
			} catch (e) {
				if (e == MissingWalletError) {
					throw new SpVuexError('TxClient:MsgSnapshotProposal:Init', 'Could not initialize signing client. Wallet is required.')
				}else{
					throw new SpVuexError('TxClient:MsgSnapshotProposal:Send', 'Could not broadcast Tx: '+ e.message)
				}
			}
		},
		async sendMsgSetDelegateAddresses({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				const txClient=await initTxClient(rootGetters)
				const msg = await txClient.msgSetDelegateAddresses(value)
				const result = await txClient.signAndBroadcast([msg], {fee: { amount: fee, 
	gas: "200000" }, memo})
				return result
			} catch (e) {
				if (e == MissingWalletError) {
					throw new SpVuexError('TxClient:MsgSetDelegateAddresses:Init', 'Could not initialize signing client. Wallet is required.')
				}else{
					throw new SpVuexError('TxClient:MsgSetDelegateAddresses:Send', 'Could not broadcast Tx: '+ e.message)
				}
			}
		},
		
		async MsgSnapshotProposal({ rootGetters }, { value }) {
			try {
				const txClient=await initTxClient(rootGetters)
				const msg = await txClient.msgSnapshotProposal(value)
				return msg
				...
				if (e == MissingWalletError) {
					...
					throw new SpVuexError('TxClient:MsgSnapshotProposal:Create', 'Could not create message: ' + e.message)
		...
		async MsgSetDelegateAddresses({ rootGetters }, { value }) {
			try {
				...
				if (e == MissingWalletError) {
					...
					throw new SpVuexError('TxClient:MsgSetDelegateAddresses:Create', 'Could not create message: ' + e.message)
